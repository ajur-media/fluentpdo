 ...
 * Class InsertTest
 *
 * @covers \AJUR\FluentPDO\Queries\Insert
 */
class InsertTest extends TestCase
{

    /** @var AJUR\FluentPDO\Query */
    protected $fluent;
    ...
    public function testInsertUpdate()
    {
        $query = $this->fluent->insertInto('article', ['id' => 1])
            ->onDuplicateKeyUpdate([
                ...
                'title'   => 'article 1b',
                'content' => new AJUR\FluentPDO\Literal('abs(-1)') // let's update with a literal and a parameter value
            ]);
    ...
    public function testInsertWithLiteral()
    {
        $query = $this->fluent->insertInto('article',
            [
                'user_id'    => 1,
                'updated_at' => new AJUR\FluentPDO\Literal('NOW()'),
                'title'      => 'new title',
                'content'    => 'new content',
            ]);
