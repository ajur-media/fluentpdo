...
use AJUR\FluentPDO\{Exception, Literal, Utilities};
...
abstract class Common extends Base
{
    ...
    private function addJoinStatements($clause, $statement, $parameters = [])
    {
        ...
        if (stripos( $statement, ' ON ' ) !== false || stripos( $statement, ' USING' ) !== false) {
        ...
        $matches[ 1 ][] = $lastItem; // was array_push( $matches[ 1 ], $lastItem );
